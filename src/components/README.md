...
| `getImage`       | Создание токена изображения | `import { getImage } from 'umbot'`       |
...
| `selectedElement` | Поиск выбранного элемента по значению                                     | `elements`: массив элементов<br>`text`: искомое значение<br>`keys?`: ключ для поиска в объекте<br>`thisPage?: number`: номер текущей страницы |
| `getPageNav`      | Подписи кнопок пагинации (`['👈 Назад', 'Дальше 👉']` или номера страниц) | `isNumber?`: boolean — показывать номера страниц вместо стрелок                                                                               |
| `getPageInfo`     | Информация о текущей странице: `"N страница из M"`                        | —                                                                                                                                             |
| `getMaxPage`      | Количество страниц                                                        | `elements?`: массив элементов (если не передан — используется внутренний кэш)                                                                 |
