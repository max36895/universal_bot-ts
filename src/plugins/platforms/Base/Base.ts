 ...
 *
 * Чтобы подключить свою платформу, унаследуйтесь от этого класса и реализуйте
 ...
 *
 * === Обязательные методы ===
 * - {@link isPlatformOnQuery} — определяет, относится ли запрос к вашей платформе
 * - {@link setQueryData} — парсит запрос и заполняет `controller`
 ...
 * - {@link isLocalStorage}, {@link getLocalStorage}, {@link setLocalStorage} — если платформа поддерживает сохранение состояния
    ...
    implements IPlatformAdapter<TQuery>
{
     ...
     * Используется только при локальном тестировании. Необходимо указывать для того, чтобы можно было корректно проверить работоспособность приложения.
     *
