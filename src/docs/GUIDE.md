...
- **Производительность.** Обработка запроса внутри фреймворка — менее 30 мс даже при 1000 команд. Это критично для
  голосовых платформ: например, Алиса ждёт ответ не дольше 4,5 секунды.
...
6. Вызывает `platformAdapter.getContent(controller)` — формирует ответ (внутри этого метода также сохраняются
   `userData` / `state`).
...
> **Про типизацию `userData` в команде:** `addCommand` — generic-метод. Чтобы TypeScript знал про ваши поля в
> `bc.userData`, передайте интерфейс как `bot.addCommand(...(_, bc: BotController<IUserData>))`. Подробное описание всех
...
- Независимо от `messageId`: если fallback зарегистрирован, он сработает даже на первом сообщении
  (и имеет приоритет над welcome-текстом — см. [«Порядок диспетчера»](#порядок-диспетчера)).
...
5. **Внутри шага** вы опять можете:
    ...
    - Установить `thisIntentName = null` — выйти из сценария (следующий запрос пойдёт по обычному пути: команды →
      интенты → fallback).
    - Не трогать `thisIntentName` — остаться на текущем шаге (пользователь должен корректно ответить, чтобы перейти
      дальше).
...
- Лимит — 1 КБ на каждое состояние. При превышении фреймворк пишет ошибку в лог и не отправляет это поле платформе
  (сами данные в `userData`/`state` не очищаются, они просто не попадут в ответ).
- Не нужен сервер БД.
...
> **Совет:** проверяйте payload **до** intentName. Если у вас есть кнопка с title "Играть" и одновременно интент `play`,
> то при нажатии кнопки `userCommand` станет `'играть'` — и без проверки payload сработает интент вместо обработчика
> кнопки.
...
- **Токен** — от @BotFather.
- **Состояния нет** — нужна БД для `userData` (или `isLocalStorage: true`, но Telegram его не поддерживает, фреймворк
  fallback'нет на БД).
...
- **Разметка выключена по умолчанию** — `parse_mode` передаётся только при явном `telegram_parse_mode`. При HTML или
  ...
  `T_FORMAT_MARKDOWN` и `escapeMarkdownV2()`.
...
**Особенности:** pool size 50, таймауты 2s, поддержка операторов запросов (`$gt`, `$in`, `$or`, агрегации),
...
Загрузка изображений и звуков на платформу занимает 200–1000 мс. Для первого пользователя это означает долгий ответ (
возможно, превышение 3-секундного лимита голосовых платформ).
...
| `loadSounds`   | `sounds: string[]`, `platforms?: TAppType[]`                              | `Promise<string \| null>[]` | Загрузить звуки на платформы       |
...
на Алисе. У Маруси и SmartApp сырые SSML-теги в `tts` передаются как есть, но поддержка конкретных эффектов зависит
...
Адаптер вернул `false` из `setQueryData`. Скорее всего, запрос не соответствует формату платформы. Проверьте вебхук URL
и секрет.
