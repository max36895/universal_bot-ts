...
Начиная с версии `2.1.0` (и доступно в текущей `3.0.x`), вы можете заменить встроенный `fetch` на любой совместимый HTTP-клиент через
...
Это совместимо с глобальным `fetch` в Node.js 18+, а также с библиотеками типа `node-fetch`, `undici`, `got` (через обёртку).
