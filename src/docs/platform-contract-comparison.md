...
| `response.card`                       | BigImage `{type, image_id: int}`; ItemsList `{type, items: [{image_id: int}]}` | `cardProcessing` (Marusia/Card.ts): 1:1 (галерея → ItemsList), токены через `marusia.getPictureUploadLink` → upload → `marusia.savePicture` | 📄 ✅                                                                 |
