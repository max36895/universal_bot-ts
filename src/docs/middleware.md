...
- Порядок выполнения: сначала глобальная цепочка, потом платформенно-специфичная. Глобальные middleware
  выполняются «вниз» до конца своего списка, а их «код после `await next()`» срабатывает при раскрутке стека —
  то есть **после всей платформенной цепочки, в обратном порядке**. Пример с одним глобальным `mw1`
  (лог до/после `next()`) и платформенным `mw2`: порядок `1, 2, 3, 4`, где 3 — «после next()» в mw2,
  а 4 — «после next()» в mw1. С двумя глобальными `mw1a`, `mw1b` (оба с логом после `next()`): `1a, 1b, 2, 3`
  — затем при раскрутке `3b`, `3a` — то есть код после `next()` у глобальных middleware выполняется
  последним. Планируйте логику после `next()` с учётом этого: она видит состояние, уже изменённое
  платформенной цепочкой.
