...
### Какой фреймворк выбрать для создания навыка под Алису и бота Telegram одновременно?
...
- **Порядок диспетчера** (важно!). Сначала выполняется глобальная цепочка middleware (полностью, включая код после
  ...
  запускается. Дальше сам диспетчер (`controller.run()`) обрабатывает запрос по цепочке:
    1. **Событийные обработчики** (`bot.addEvent(...)`) — если адаптер распознал тип события (фото, голосовое,
       нажатие кнопки) и есть зарегистрированный хендлер.
    2. **Шаг** (`oldIntentName`, если пользователь внутри мультишага и шаг зарегистрирован).
    3. **Команды** — проверяется каждая в порядке регистрации, **первая подошедшая** побеждает и выполняется (цепочка
       прерывается).
...
- Тип поиска: Если слот — строка, ищется через `includes()`. Если нужно точное совпадение, используйте регулярное
  ...
  `userCommand` уже приведён к нижнему регистру).
...
- **Включение/выключение на уровне сборки** — `bot.clearUse()` удаляет сразу **все** плагины, адаптеры, middleware и
  платформы (это глобальная операция), поэтому для точечного управления функционалом его лучше не использовать.
...
потреблению памяти (до 4 ГБ против 400 МБ на Linux).
...
Не рекомендуется. Файловая БД (FileAdapter) хранит данные в оперативной памяти и рассчитана на быстрый старт или базы
объёмом до нескольких сотен МБ — при превышении возможен Out of Memory и падение приложения.
...
2. ✅ Доступен ли ваш сервер из интернета (для вебхуков)
3. ✅ Корректно ли настроены URL вебхука в консоли разработчика платформы
...
- Тип кнопок: у голосовых платформ (Алиса, Маруся) кнопки-ссылки (`addLink`, `hide: false`) отображаются как видимые
  кнопки, а интерактивные (`addBtn`, `hide: true`) — как саджесты и скрываются после нажатия.
...
### Бот отвечает пусто или стандартной заглушкой вместо ожидаемого текста
...
### Размер файловой БД приближается к 400 МБ
...
Практический лимит ответа для Алисы — около 3 секунд: фреймворк пишет предупреждение в лог уже после 2 секунд
обработки, а после 2,9 секунды — ошибку. У других платформ лимит отличается и может меняться.
Оптимизация:
...
- Убедитесь, что шаг зарегистрирован через bot.addStep() — сделать это можно в любой момент, главное до того, как
  пользователь окажется в этом шаге.
