...
- **Порядок диспетчера** (важно!). Входящий запрос сначала проходит через слой **middleware** (глобальные →
  платформенные); если какое-то middleware не вызвало `next()`, диспетчер вообще не запускается. Дальше сам диспетчер
  (`controller.run()`) обрабатывает запрос по цепочке:
    1. **Шаг** (`oldIntentName`, если пользователь внутри мультишага и шаг зарегистрирован).
    2. **Команды** — проверяется каждая в порядке регистрации, **первая подошедшая** побеждает и выполняется (цепочка
       прерывается).
    3. **Интенты** из `platformParams` (включая встроенные `welcome`/`help`).
    4. **FALLBACK_COMMAND** (`*`).
