...
Выбор платформы происходит автоматически в зависимости от запроса, который пришел в приложение, главное не забыть подключить адаптеры для платформ.
...
2. Получите OAuth токен в [Яндекс.OAuth](https://oauth.yandex.ru) если он необходим. Токен нужен для загрузки аудио или изображений.
...
class AlisaController extends BotController {
    public action(intentName: string | null): void {
        if (intentName === WELCOME_INTENT_NAME) {
            ...
            this.text = `Привет, ${his.nlu.getUserName()?.first_name || 'пользователь'}!`;
...
class MaxController extends BotController {
    public action(intentName: string | null): void {
        if (intentName === WELCOME_INTENT_NAME) {
            ...
            // Добавление клавиатуры
            this.buttons.addBtn('Меню').addBtn('Помощь').addBtn('О нас', 'https://dev.max.ru/docs/chatbots/bots-create');
...
Благодаря такому подходу вы можете сосредоточиться на бизнес-логике вашего бота или навыка,  
а не на деталях реализации под каждую платформу. Один код — работает везде.
