...
3. Поддержка активных рассылок - Сейчас приложение работает по принципу Запрос → Ответ, из-за чего нет возможности
...
8. Шаги (добавленные через `addStep`), привязанные к имени шага или сценария (через `controller.oldIntentName`),
...
| Переопределить ответ webhook   |   готово    | В методы run и webhook добавлен 3-й callback-аргумент                                                                            |
