...
использования(Количество команд до 500 000).
...
Это означает, что при _первом_ вызове `run()` с командами, использующими новые паттерны, происходит \*
_компиляция `RegExp`\*\*, что занимает больше времени. При последующих вызовах с теми же паттернами, _
