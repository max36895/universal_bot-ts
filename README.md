...
все ведущие российские платформы и не только: `Яндекс.Алиса`, `Маруся`, `Сбер SmartApp`, а также `Telegram`, `VK`, `MAX`
и `Viber` из коробки.
...
> Ваша логика — одна. Платформа — любая.

**Ключевые преимущества:**

- ✅ **Алиса + Маруся + Сбер SmartApp + ...** — одновременно, без лишних усилий.
- ⚡ В типичных сценариях (до 1 000 команд) внутренняя обработка `umbot` занимает в среднем < 30 мс. (Оставляет минимум
  ...
  При первичной загрузке медиафайлов время ответа может превысить 1 секунду — поэтому рекомендуется использовать
  предзагрузку (класс `Preload`).
...
Для корректной установки на Windows нужно следовать [инструкции](https://github.com/nodejs/node-gyp#on-windows),
