...
- Добавлена новая поддержка re2 для обработки регулярных выражений. Благодаря этому потребление памяти может
...
- Для удобства, константа FALLBACK_COMMAND стала иметь значение "\*", данный подход позволяет просто указать "\*", чтобы
  указать команду для действия, когда нужная команда не была найдена
