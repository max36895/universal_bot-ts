...
- Добавлена новая зависимость re2 для обработки регулярных выражений. Благодаря этому потребление памяти сократилось, а также время обработки регулярнах выражений ускорилось примерно в 2-6 раз

### Обновлено

- Ошибки во время работы приложения записываются как ошибки, а не как обычные логи
- Немного оптимизирована логика поиска нужного текста
...
- Удалены все устаревшие методы
- Метод bot.initBotController принимает другие аргументы
...
- Ошибка в benchmark из-за чего он показывал результат лучше чем есть на самом деле
