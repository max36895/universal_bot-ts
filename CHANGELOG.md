...
- **Платформы**: malformed-запросы без обязательных полей не вызывают TypeError и 500: `initUserCommand` защищает отсутствие `original_utterance` (Алиса/Маруся), Telegram-адаптер — отсутствие `chat` в апдейте (такие апдейты пропускаются с `skipAutoReply`).
