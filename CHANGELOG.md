...
- **[Breaking] Telegram**: `parse_mode` по умолчанию изменён с `MarkdownV2` на `HTML`. Разработчикам, использующим `MarkdownV2`, необходимо явно указывать его в параметрах запроса.
- **Telegram**: устаревший режим `markdown` заменён на `MarkdownV2`. Добавлено автоматическое базовое экранирование HTML-символов (`&`, `<`, `>`) для защиты от XSS.
...
- **VK**: исправлена обработка `message_event`: адаптер теперь использует обязательный `messages.sendEvent` вместо `messages.send` (устраняет ошибку "Бот недоступен" у пользователя).
- **VK**: добавлена защита от инъекции произвольных свойств через spread-оператор: ключи `action`, `color`, `hash`, `_group` защищены от перезаписи.
- **VK**: исправлена некорректная валидация payload (255 байт): проверка теперь выполняется строго после сериализации.
- **VK**: исправлен поиск hash для кнопок `vkpay` при передаче строкового payload.
- **Telegram**: добавлена проверка длины `callback_data` (лимит 64 байта). При превышении данные безопасно обрезаются с записью предупреждения в лог.
...
[3.0.15]: https://github.com/max36895/universal_bot-ts/compare/v3.0.0...v3.0.15
