...
- **Бенчмарки (методика v3)**: стенды `npm run compare*` переведены на честную методику замера. (1) Фазы латентностей и памяти разделены: вся фаза латентностей идёт без единого вызова GC, фаза памяти (с forceGC) запускается только после её завершения — пока фаза памяти стояла в том же раунде, каждый следующий раунд наследовал только что скомпактированную кучу, и pooled p50 взлетал у обеих сторон в ~1.5 раза (матрица методик, VK, 6 команд: forceGC перед замером 3.0/2.2 мкс; фаза памяти в раунде 3.4/3.3; без GC вовсе 2.3/2.3; steady-эталон 1.7/1.2) — GC в замере измерял чувствительность к compaction V8, а не стоимость маршрутизации. (2) Интерливинг раундов: в каждом раунде все участники идут по очереди, тепловой дрейф CPU распределяется на всех поровну. (3) После регистрации команд все участники ждут единую паузу 100 мс: хвосты инициализации (снимок команд umbot, компиляция RegExp, recompose-цепочек конкурентов) не попадают в замер — раньше только umbot имел delay, а конкуренты стартовали сразу. (4) Победа помечается только при отрыве >5% и >0.3 мкс; при меньшей разнице строки помечаются «паритет» — раньше при равных p50 метка «← лучший» висела сразу у обоих участников. Свежие результаты v3: Telegram — umbot лучший во всех 12 сценариях (включая оба «6-командных»: 1.3 против 3.1 мкс у grammy на exact), VK — 7 из 13 (+1 паритет), Viber — 12 из 13, MAX — 11 из 13; контрольный steady-замер «1000 fallback» — umbot 31.3 против 53.1 мкс у vk-io. Подробные таблицы — в BENCHMARKS.md.
- **Бенчмарки (методика v4 — прогрев до плато)**: фиксированный прогрев 2×500 запросов ловил участников на крутом участке кривой tier-up/inline-кэшей V8 — «толстый» фреймворк (у umbot контроллер на 30 полей и больше полиморфных call-site'ов) выходит на плато позже и выглядел медленнее собственного steady-state. Замер кривых прогрева на VK («6 команд, точное совпадение», медианы по 5 раундам): umbot 1.94 → 1.37 → 0.89 мкс (при 500/1000/2000 прогретых запросов), vk-io 2.11 → 1.62 → 0.85 — при прогреве ≥4000 запросов разрыв 0.4 мкс схлопывался в паритет 0.85 против 0.85 мкс. Теперь `runner.js` перед каждым замерным раундом гоняет участника циклами по 500 запросов, пока медиана цикла перестаёт улучшаться (до 20 циклов). Свежие результаты v4: Telegram — 13 из 13, Alisa — 13 из 13, Viber — 13 из 13, MAX — 13 из 13, VK — 8 из 13 (+1 паритет; vk-io быстрее только на «6 команд» и «50 fallback» — реальная цена пайплайна, а не артефакт). Заодно рефакторинг стендов: Telegram-стенд `competitors.js` (дублировал каркас целиком) заменён на `telegram.js` в едином формате платформенных файлов, добавлен сценарий «50 команд, fallback» для всех стендов.
